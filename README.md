...
大型实验固定使用统一脚本，不手工逐组运行。默认完整套件包含 39 个变体、5 个随机
种子，共 195 组配置：
...
如果只想先运行最关键的 13 个变体、共 65 组，可以使用核心套件：

```bash
.venv-gnn/bin/python scripts/run_gnn_ablation.py --suite core
```

核心套件完成后再运行默认完整套件，已经完成的同名实验会直接复用，不会重复训练或
评测。完整套件额外覆盖扩散步数 `8、12、20`，重启系数 `0、0.2、0.6、0.8`，步数与
