...
| 4. OD 条件化双向 NBFNet | 进行中，基础实现与 CUDA 烟雾训练已完成 | 完成 GPU GraphSAGE 节点打分和核心 72 组验证集消融；第二版已完成精确端点局部接入、固定候选、1,200×2,000 真实标签、128 个需求原型、五种子无传播 MLP，以及采用全原型精确混合梯度的基础双向 NBFNet。下一步在 5060 Ti 上运行冻结配置的正式五种子训练与核心消融。 |
...
并在 RTX 3050 4GB 上以 32 维、6 层正式结构跑通 1 epoch；这只证明执行链路成立，不是
模型效果结论。下一步在 5060 Ti 上运行冻结配置的正式五种子训练。详细口径与证据见
