...
当前参加 AIC 算法创新赛的正式赛题为 **AI+学科交叉**。项目按“交通工程/智慧交通科研
方法 + 图算法与 AI”定位，不开发独立交互平台；赛题契合度、交付清单和论文路线见
[`赛题_AI+学科交叉.md`](赛题_AI+学科交叉.md)。

方法边界已经完成文献复核：底层“区域受限 Dijkstra + 边界 shortcut + 删除内部节点后的精确
覆盖图查询”采用成熟的 CRP/Graph Separator 式结构，不作为原创压缩算法；本项目的差异化核心
是历史 OD 驱动的**选择性区域物化、精确区域收益排序、无路径输入、时空无泄漏协议和严格不重叠
部署闭环**。详细边界见
...
| 8. C++ 高性能复核 | 已完成 | 同一 C++20/O3 实现完成两城 800,000 个正式计时配对（含 BRIDGE-B）；Porto 平均加速稳定，Chicago 受 shortcut 扫边开销限制。 |
| 9. BRIDGE-B 预算感知增强 | 已完成 | Top-K 收益、shortcut 预算和区域冲突进入神经目标；两城三种子保住全局/头部并降低 shortcut，在线展开与 Z0 差异均不足 3%。 |
| 10. 空间与查询尺度诊断 | 已完成 | 两城 Y/F 空间收益图与查询工作量四分位均已完成；Porto BRIDGE-B 广泛改善，高工作量查询显著摊薄固定索引接入成本。 |

各阶段的目标、成果、证据、遗留问题和完成标准统一记录在
[`reports/`](reports/README.md) 中。
最终统一结论、论文主张、局限性和投稿判断见
...
[`reports/阶段五_梯度稳定化与Z0神经残差.md`](reports/阶段五_梯度稳定化与Z0神经残差.md)。
第二版到批量持续学习的版本边界见
[`reports/阶段四_持续学习闭环演进路线.md`](reports/阶段四_持续学习闭环演进路线.md)。

第二版最终 `K=18` 严格非重叠多区域在线结果（索引端点缓存关闭）：

| 选区方法 | 当前耗时变化 | 当前展开变化 | 未来耗时变化 | 未来展开变化 | 正确率 |
| --- | ---: | ---: | ---: | ---: | ---: |
| 随机 | -0.61% | -4.91% | -0.55% | -4.73% | 100% |
| 历史热点 | -7.36% | -15.84% | -7.74% | -15.90% | 100% |
| 中点 Proxy | -8.89% | -17.35% | -9.35% | -17.66% | 100% |
| Z0 固定传播 | -8.55% | -17.46% | -9.19% | -17.76% | 100% |
| G3 三种子平均 | -9.50% | -18.42% | -9.95% | -18.64% | 100% |

G3 相对 Z0 的展开节点增益约为 1 个百分点，但 K=18 的 shortcut 数为
...
`6.51% / 7.48%`，G4 三种子均值下降 `6.41% / 8.90%`。Chicago 的优势主要集中于尾延迟：
约 20% 的展开节点减少使 20 个组合中 14 个 P95 下降；平均耗时改善则受密集 shortcuts 的
`18%～28%` 额外扫边开销限制。
BRIDGE-B 的三种子补测中，Porto 平均耗时下降 `6.06% / 7.85%`；Chicago 平均增加
`4.64% / 3.89%`，再次确认扫描边而非距离正确性是跨城瓶颈。
完整结论见 [`reports/阶段八_C++高性能在线评测.md`](reports/阶段八_C++高性能在线评测.md)。

预算感知增强 BRIDGE-B 的三种子冻结复现也已完成。Porto 三个种子选择完全相同的 Top-18
成员，shortcut 从 Z0 的 `6,712` 降至 `6,644`，当前/未来每查询少展开约 `75/76` 个节点；
Chicago shortcut 从 `23,378` 降至 `22,581～23,104`，当前/未来多展开约 `54～92` 个节点。
相对每查询数千个节点的搜索规模，两城与 Z0 的展开差均不足 3%，可视为真实工作量基本持平；
其主要成果是稳定保住全局排序和头部质量，同时显式降低索引成本。完整记录见
[`reports/阶段七_成本感知神经部署优化.md`](reports/阶段七_成本感知神经部署优化.md)。

冻结空间诊断进一步显示，Porto BRIDGE-B 在 Y/F 分别改善 `87.76% / 90.00%` 的有效米制等边方格；
Chicago 虽存在局部改善，但不形成城市平均优势。按原图展开量分层后，Chicago 最高工作量四分位
仍减少 `24.92% / 25.72%` 的展开节点，而最低四分位产生额外开销。由此，当前框架更适合出租车、
驾车等中长距离或高搜索工作量查询；短步行/骑行式负载是明确的适用边界。完整证据见
[`reports/阶段九_空间收益异质性与部署覆盖.md`](reports/阶段九_空间收益异质性与部署覆盖.md)。
...
同 split 的第一版 midpoint Proxy holdout Spearman 为 `0.8918`，收益@5/10/18 为
`170.043 / 149.592 / 142.332`。纯传播在 Spearman 和 K=5/10 上更好，但两者 Top-18
候选集合完全相同、Top-18 收益也相同；因此当前复杂模型的增益主要体现在全局排序和较小 K，
还不能表述成 K=18 固定预算下超过简单 Proxy。
