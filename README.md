...
| 4. OD 条件化双向 NBFNet | 纯传播筛选与旧协议重复已完成 | 空间重叠组隔离 split 上的四组 32 层纯传播筛选完成；旧协议五种子暴露明显优化不稳定，尚不能证明多种子稳定性。 |
| 5. 最终对比实验 | 进行中 | 新 split 五种子 MLP 已完成；还需先修复纯传播训练协议，再补基础 NBFNet、端点密度、随机拓扑纯传播和未来窗口。 |
