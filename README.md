...
| 4. OD 条件化双向 NBFNet | 进行中，基础模型正式五种子已完成 | 完成精确端点局部接入、固定候选、1,200×2,000 真实标签、128 个需求原型、五种子无传播 MLP 和 RTX 5060 Ti 基础 NBFNet 五种子训练。NBFNet 的 holdout NDCG@K 为 0.9821，Top-K 收益为 129.051，均超过 MLP 与原始频率；服务器正在运行单种子核心传播与拓扑消融筛选，消融尚无最终结论。 |
...
但排序顶部质量超过 MLP 和原始频率；当前结论支持进入核心传播与拓扑消融，尚不能单独
证明道路拓扑贡献。MLP 口径见
...
当前服务器正在运行单种子核心传播与拓扑消融筛选。筛选完成并复核全部对照之前，不能
宣称基础 NBFNet 已经学到正确道路拓扑；服务器运行中的即时指标也不替代正式汇总报告。
