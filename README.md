...
| 4. OD 条件化双向 NBFNet | 进行中，基础实现与 CUDA 烟雾训练已完成 | 完成 GPU GraphSAGE 节点打分和核心 72 组验证集消融；第二版已完成精确端点局部接入、固定候选、1,200×2,000 真实标签、128 个需求原型、五种子无传播 MLP，以及按原型分批的基础双向 NBFNet。下一步在 5060 Ti 上运行冻结配置的正式五种子训练与核心消融。 |
| 5. 最终对比实验 | 未开始 | 需要在同一固定候选和预算下比较 NBFNet、MLP、Proxy、频率与随机排序，并完成消融、稳定性和跨城市实验。 |
...
第二版已完成内部端点局部接入、固定候选、真实收益标签和统一数据接口。接口保存 128 个
只由历史 OD 与静态道路图构造的需求原型；NBFNet 将从需求原型注入状态，沿
有向道路逐层传播，学习边消息、传播门控、不同传播深度和
入口—出口区域读出。核心对比必须包括原始频率、第一版固定扩散、GraphSAGE、单向需求场、
无向传播、打乱 OD 配对和无消息传递 MLP。

基础 NBFNet 通过核心消融后，第二版继续验证两项深层改进：按 `1、2、4、8...` 扩大感受
范围的倍增传播，以及允许深层状态选择性读取早期状态的跨层注意力残差。两项改进必须分别
消融，再测试组合效果；倍增连接只由静态道路图构造，不使用历史最短路径。
...
低于原始频率的 `128.530`。阶段状态仍为 `ready_for_nbfnet`，因为五个种子的排序相关性
均为正且 Top-K 都显著高于候选均值，但不能表述成 MLP 全面超过频率基线。下一步直接实现
已经确定的 OD 条件化双向
NBFNet。详细口径与证据见
