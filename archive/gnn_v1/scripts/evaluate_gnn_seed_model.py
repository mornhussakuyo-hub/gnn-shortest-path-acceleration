...
DEFAULT_SCORE_CSV = ROOT_DIR / "archive" / "gnn_v1" / "results" / "gnn_v1" / "node_scores.csv"
DEFAULT_OUTPUT_DIR = ROOT_DIR / "archive" / "gnn_v1" / "results" / "gnn_v1"
    ...
    for method, build_regions in strategies:
        ...
        row = {
            ...
            "endpoint_access_rate_pct": (
                endpoint_access_count / len(evaluation_queries) * 100.0
            ),
            "endpoint_cache_capacity": args.endpoint_cache_capacity,
            ...
            "indexed_avg_access_expanded": statistics.mean(
                metrics["indexed_access_expanded"]
            ),
            "indexed_avg_graph_expanded": statistics.mean(
                metrics["indexed_graph_expanded"]
            ),
        ...
        print(
            ...
            f"endpoint_access={row['endpoint_access_rate_pct']:.2f}% "
            f"cache_hit={row['endpoint_cache_hit_rate_pct']:.2f}% "
...
def _write_report(path: Path, rows: list[dict[str, object]]) -> None:
    lines = [
        "# 第一版 GNN 种子模型精确配对评测",
        "",
        "| 方法 | 区域数 | Shortcut | 端点接入率 | 缓存命中率 | 回退率 | 平均耗时变化 | P95 变化 | 展开节点变化 | 正确率 |",
        "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
    ]
    for row in rows:
        lines.append(
