...
from archive.gnn_v1.src.gnn_data import FEATURE_NAMES
...
def _load_training_dependencies() -> None:
    global SeedValueGraphSage, SeedValueMlp, build_gnn_data, np, torch
    try:
        import numpy as np
        import torch

        from archive.gnn_v1.src.gnn_data import build_gnn_data
        from archive.gnn_v1.src.gnn_model import SeedValueGraphSage, SeedValueMlp
    except ModuleNotFoundError as error:
        raise SystemExit(
            "缺少第一版 GNN 训练依赖；请安装 archive/gnn_v1/requirements.txt"
        ) from error
