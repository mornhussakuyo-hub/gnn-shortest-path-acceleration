...
形成显著偏向轻量索引的配置，六组 Y/F 相对 \zmethod 多展开约 28--110 个节点；Chicago 将
shortcut 从 23,378 降至 13,049--15,122，其中 seed 43、$\lambda=0.3$ 在 Y/F 同时略优于
\zmethod。12 个配置中有 1 个达到两窗口联合在线门，说明固定 $\lambda$ 对城市和种子较敏感。
24 个方法--窗口在线结果的距离正确率均为 100\%。E0 因而是有效且幅度充足的单目标成本旋钮；
\bmethod 则在此基础上把成本控制扩展为头部收益、区域冲突与完整排序的联合优化，并在 Porto
实现三个种子完全一致的部署成员和 Y/F 稳定改善。这一对照表明联合训练获得了事后分数减法尚未
提供的跨种子部署一致性。E0 更低的 shortcut 与 \bmethod 更稳的多目标表现也揭示了两者不同的
Pareto 偏好，而非简单的单指标替代关系。
