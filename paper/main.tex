...
但六个配置的 Y/F 展开量均劣于 \zmethod；Chicago 将 shortcut 从 23,378 降至
13,049--15,122，但也只有 seed 43、$\lambda=0.3$ 在 Y/F 同时略优于 \zmethod，其余 11 个
城市--种子--权重配置均未同时通过。24 个方法--窗口在线结果的距离正确率均为 100\%。因此，
固定线性惩罚确实提供有效的成本旋钮，却不能稳定保护真实部署收益；它没有复现后续 \bmethod
在 Porto 的跨种子一致改善。这一对照支持把头部收益、成本和冲突放入联合训练目标，而不是把
\bmethod 简化为事后分数减法；但 E0 的 shortcut 更低，本文不声称 \bmethod 在所有成本指标上
支配该线性基线。
