...
残差注意力和倍增传播不属于本轮基础筛选；筛选完成后，它们已作为严格纯传播证伪实验的
四组受控变体实现。

## 当前状态

基础 NBFNet 正式五种子和固定种子 44 的单种子筛选均已完成。度保持随机重连、无向图和
去边特征模型重新训练后与基础模型几乎相同，多数变体还选出同一个 Top-18。结论不是训练
失败，而是“当前基础结构没有证明正确道路拓扑的独立价值”。进一步诊断发现区域特征直通、
第 0 层读出、6 层感受野不足、候选切分重叠和 Top-K 饱和等混杂因素。完整数字与下一轮
设计见 [`阶段四_NBFNet传播诊断与深层纯传播实验.md`](阶段四_NBFNet传播诊断与深层纯传播实验.md)。
后续四组空间隔离的 32 层纯传播压力测试也已完成：传播表示本身能够预测当前候选价值，
并按 validation 选择 `propagation_doubling`；正确道路拓扑必要性仍待同口径随机拓扑对照。
...
| `shuffled_od` | 半周期保测度耦合打乱配对，同时保持起点和终点边际 | 正确 OD 配对是否重要 |
...
checkpoint 和结构选择只使用 validation Spearman；holdout Top-K 平均真实收益、NDCG@K
和 Spearman 只在选择完成后报告，不参与选型。最低证据链为：
...
基础模型没有稳定超过关键拓扑对照。项目随后进行一次范围受控的严格纯传播证伪实验：切断
直通、隔离空间重叠、匹配 32 跳感受野并延长训练。四组实验现已完成并通过压力测试；按
validation 选择 `propagation_doubling`，但随机拓扑纯传播和同新 split 无传播基线尚未补齐。
