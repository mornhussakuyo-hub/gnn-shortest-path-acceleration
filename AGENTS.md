...
3. **S3 已完成**：完全冻结 S2 协议，两城 seeds 42/43/44 均完成 holdout 与 K=18 Y/F 在线门。
4. S3 证明 BRIDGE-B 能稳定保持/改善全局排序与头部、控制 shortcut 成本，并在 Porto 稳定改善
   真实展开量；但“两城真实展开量均稳定超过 Z0”的联合门未通过，不启动同目标长训。
5. 下一轮若继续优化，应直接建模多区域集合交互、扫边或在线工作量代理，并使用新的未来窗口或
   未见城市做确认；不得在已解锁 Y/F 上继续调参后包装成首次确认。
6. 任何不利结果均保留并写入阶段日志；不事后删除种子、epoch 或在线方法。
