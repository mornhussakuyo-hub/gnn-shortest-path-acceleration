...
- S3 两城 seed 43/44 的训练、冻结全候选推理与 K=18 Y/F 精确在线评测已全部完成并回传本机。
  两城新增 16,000 次配对与 S2 seed 42 合计 24,000 次配对，距离正确率均为 100%。
- Porto seeds 42/43/44 选出的 Top-18 成员集合完全相同（仅分数次序有微小交换），shortcuts
  均为 `6644`；Y/F 展开量均为 `8906.302/8665.288`，相对 Z0 稳定少
  `75.478/76.202`，在线展开量三种子标准差为 0。
- Chicago seeds 42/43 选出相同成员集合，seed 44 部分变化；三种子 shortcuts 为
  `23104/23104/22581`，均低于 Z0 的 `23378`。三种子 holdout Spearman 和 NDCG@5 均高于
  Z0，但 Y/F 展开量均值为 `2675.864/2602.146`，相对 Z0 仍多 `65.947/72.032`。
...
1. BRIDGE-B 两城三种子的 C++20/O3 同实现补测已经完成：Porto 六组平均耗时全部下降；Chicago
   六组平均耗时均上升，原因仍是扫描边增加约 23%--25%，所有距离正确率为 100%。
2. 阶段九空间诊断已经完成。按 16×16 米制等边方格重绘后，Porto BRIDGE-B 在 Y/F 分别改善 87.76%/90.00% 的有效方格，属于
   头部与非头部同时改善的收益扩展；Chicago 为 41.18%/43.33%，不通过稳定空间扩展门。
3. 查询尺度分层已经完成。Chicago 原图展开最低四分位使用 BRIDGE-B 会产生额外工作量，最高
   四分位在 Y/F 则减少 24.92%/25.72%；Porto 最高四分位每查询少展开约 5,039/4,843 个节点。
4. 可写“选择性区域物化更适合出租车、驾车等中长距离或高搜索工作量查询，短步行/骑行负载的
   收益有限”；不能把两城对比写成交通方式因果证明，也不能写 BRIDGE-B 在 Chicago 长查询超过 Z0。
5. 用户已经决定研究收口，不再实现原计划的简单 Z0 成本启发式，不启动 BRIDGE-B2，也不继续在
   已解锁 Y/F 上调参。除非教师或审稿意见提出明确证据需求，后续只做论文整理与期刊适配。

## 阶段七时间预算

- 32 层 G3/BRIDGE 单 epoch 实测约 60～70 秒；S1 两城 12 epoch 并行约 15～20 分钟。
- 冻结全候选导出约数分钟；两城 K=18、Y/F 在线复核的时间视现有缓存与服务器负载而定。
- S3 重复种子已完成；由于跨城市真实展开量联合门未通过，不启动同目标长训或五种子扩展。
...
- S0--S3 两城训练、冻结全候选和 K=18 在线产物已回传本机，位于两城
  `results/.../g5_cost_aware_exploration/`；launcher/PID 与逐 epoch 快照不宽泛提交。
- 当前详细执行记录：`reports/阶段七_成本感知神经部署优化.md`。
- 空间收益地图、头部/非头部与查询尺度诊断：`reports/阶段九_空间收益异质性与部署覆盖.md`。
