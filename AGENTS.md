...
1. BRIDGE-B 两城三种子的 C++20/O3 同实现补测已经启动；只补墙钟、P95、扫描边和正确率，
   不重新训练或覆盖原 Z0/BRIDGE 结果。
2. 随后执行 `reports/阶段九_空间收益异质性与部署覆盖.md`：对 BRIDGE 和 BRIDGE-B 均绘制 Porto /
   Chicago、Y/F 的改善、持平和退化空间图，Z0 为统一参照。
3. 空间热点只由历史 H 定义；主图使用 OD 中点，起点/终点进入附录。必须报告逐查询差、网格均值、
   改善比例、种子一致性和头部/非头部统计，不得只展示有利区域。
4. 只有非头部平均增量与改善网格比例在 Y/F 均为正、且至少 2/3 种子同向，才可写“牺牲少量
   头部收益换取更广空间覆盖”；否则按收益扩展、空间重新分配或无稳定改善如实定性。
5. 地图是已解锁 Y/F 上的机制诊断，不是全新时间确认；不得依据地图继续修改 BRIDGE-B 后再把
   同一窗口包装为测试集。
6. 地图完成后再实现一个预先冻结的简单 Z0 成本启发式，检验 BRIDGE-B 是否能超过线性成本惩罚。
7. 任何不利结果均保留并写入阶段日志；不事后删除种子、窗口、网格或在线方法。
...
- 空间收益地图与头部/非头部诊断计划：`reports/阶段九_空间收益异质性与部署覆盖.md`。
